...
class LinkedBST(AbstractCollection):
    ...
    def replace(self, item: int, new_item: int):
        """
    ...
    def _height(self, node, height=0):
        ...
        if node.left != None:
            left_height = self._height(node.left, height)

        if node.right != None:
            right_height = self._height(node.right, height)
    ...
    def is_balanced(self):
        ...
        def is_balanced1(top):
            if top is None:
                return True
            return is_balanced1(top.right) and \
            is_balanced1(top.left) and \
            abs(height1(top.left) - height1(top.right)) <= 1

        return is_balanced1(self._root)


    def range_find(self, low: int, high: int):
        '''
        ...
        :return:
        '''
        return sorted([n for n in self if low <= n <= high])
    ...
    def rebalance(self):
        ...
        while len(tree_data) != 0:
            half_length = len(tree_data) // 2
            index = half_length - 1 if len(tree_data) % 2 == 0 else half_length
            item = tree_data[index]
            self.add(item)
            tree_data.pop(index)
    ...
    def successor(self, item: int) -> int:
        ...
        tree_data = []
        for d_1 in self:
            tree_data.append(d_1)
        if len(tree_data) == 0:
            return None
        return min([n for n in self if n > item])
        '''
        results = []
        for d_1 in self:
            if d_1 > item:
    ...
    def predecessor(self, item: int) -> int:
        """
